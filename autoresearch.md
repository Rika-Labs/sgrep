...
- Kept: expanded query tokenization for `-ing` / `-ed` words to include safe extra lexical variants (e.g. `scoring` -> `score`, `cached` -> `cache`) while keeping PRF compactness logic on the base token count. This improved lexical recall for exact code-search phrasing and raised quality_mrr to 84.00.
- Kept: added a lexical-confirmation bonus when strong BM25 evidence aligns with chunk-local symbol overlap. This selectively rescues exact identifier/module hits without broadly shifting semantic weighting and raised quality_mrr to 85.33.
- Kept: added an IDF-weighted high-signal field bonus for rare query terms matched in path and chunk-local symbol identifiers. This improved module/file discovery and raised quality_mrr to 87.33.
- Kept: added a documentation heading bonus using overlap between query terms and the document title / first headings. This made docs retrieval much more precise, fixed the remaining docs miss, and raised quality_mrr to 89.33.
- Discarded: broad adjacent-term query abbreviation expansion (e.g. `memory mapped` -> `mmap`) improved one hard miss but regressed overall quality. Any future abbreviation work should stay more targeted and structurally gated.
